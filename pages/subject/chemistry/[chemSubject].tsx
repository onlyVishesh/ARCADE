...
function Model({ path, size }) {
  const gltf = useLoader(GLTFLoader, `/chemistry/${path}/scene.gltf`);
  return (
    <>
...
export default function ChemistrySubject() {
  ...
  const { chemSubject } = router.query;

  const { locale } = router;
  const t = locale ? locales[locale] : locales["en"];

  const subject = chemPageInfo.find((item) => item.ChemSubject === chemSubject);
          ...
          <div className="mt-16 md:mt-0 md:col-span-1 p-4">
            <div className="h-full md:flex md:flex-col md:justify-center">
              ...
              <div className="grid place-items-center pb-8 mx-8">
                <Image
                  src={`/chemistry/${subject.QrCodeUrl}`}
                  width="250"
                  height="250"
                  alt="Molecule QR"
