...
function Model({ path, size }) {
  const gltf = useLoader(GLTFLoader, `/space/${path}/scene.gltf`);
  return (
    <>
...
export default function SpaceSubject() {
  const router = useRouter();
  const { spaceSubject } = router.query;

  const { locale } = router;
  const t = locale ? locales[locale] : locales["en"];

  const subject = spacePageInfo.find(
    (item) => item.SpaceSubject === spaceSubject
  );
    ...
    <div className="pt-20 md:pt-24 text-white">
      ...
      >
        <div className="grid w-full py-10 place-items-center">
          ...
          <div className="inline-flex h-1 bg-purple-600 rounded-full w-44"></div>
          <div className="font-fontVollkorn text-justify text-xl w-86 mt-2 mx-4 md:mx-44">
            {t[subject.ModelDetails]}
          </div>
        </div>
            ...
            <div className="h-full md:flex md:flex-col md:justify-center">
              ...
              <div className="grid place-items-center pb-8 mx-8">
                ...
                <a href={subject.ARlink} target="_blank" rel="noreferrer">
                  <button className="h-12 px-8 text-base font-semibold tracking-wider text-white border rounded-full shadow-sm mt-16 bg-red-50 bg-gradient-to-r from-secondary to-tertiary hover:shadow-lg md:hidden">
                    {t.modelViewInAR}
                  </button>
                </a>
              </div>
